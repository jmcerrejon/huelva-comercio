...
(function constructor() {
	(OS_ANDROID) ? $.tabGroup.addEventListener('androidback', function () {}) : $.tabGroup.hideNavBar();
...
if (OS_ANDROID) {
	$.tabGroup.addEventListener('open', () => {
		(_.isNull(Alloy.Globals.androidDataPush)) || notification(Alloy.Globals.androidDataPush)
	});
}
...
function changeNavBar(index) {
	...
	if (!_.isNull($[tabStacks[index].winId].getViewById('title'))) {
        ...
        $[tabStacks[index].winId].getViewById('btnRight').visible = tabStacks[index].hasSettingsEnabled;
    }
}
...
function resume(e) {
	...
	if (checkValidUser()) {
		Alloy.Globals.Api.me(function (response) {
			...
			closeSession();
			setTimeout(() => {
				Alloy.Globals.showMessage('Por motivos de seguridad, se ha cerrado la sesión.');
			}, 1000);
		});
	}
...
function checkValidUser() {
	return (Alloy.Globals.online) && (Alloy.Globals.token !== null) && (!Alloy.Globals.guest);
}

function isDiffTimePassed() {
	return Alloy.Globals.moment().diff(timeSinceBackgroundApp, 'minutes') >= time2RefreshNewsAndEventsInMinutes;
}
