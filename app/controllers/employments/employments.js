...
function changeCandidateButtonTitle() {
    ...
    item.candidateButton.title = (Alloy.Globals.candidate !== null) ? 'EDITAR MI CURRÍCULUM' : 'SUBIR MI CURRÍCULUM';
...
function handleViewNoItems({ listview, hasItems, vwNoItems, searchBar, vwSector }) {
...
function changeSectorEvent() {
    $.filterSectorCandidates.on('change', (item) => {
        ...
        search['candidates'].sectorId = item.val === 'Todos los sectores' ? '' : item.val;
    ...
    $.filterSectorEmployments.on('change', (item) => {
        ...
        search['employments'].sectorId = item.val === 'Todos los sectores' ? '' : item.val;
...
function init({ model, currentPage, lastPage, response, listView, infScrollWidget }) {
...
function renderSectorList(sectors, firstElementName) {
    ...
    sectors.map((sector) => {
        ...
        (firstElementName !== null && firstElementName === sector.name) ? result.unshift(item) : result.push(item);
    });
...
function reset(item) {
    ...
    resetListView(index === 'employments' ? {
        model: 'employments',
...
function resetListView({ model, listview, currentPage, infScrollWidget, refreshControl, vwNoItems, searchBar, vwSector }) {
...
function openView({model, index, path}) {
    const data = _.isUndefined(Alloy.Collections[model].get(index)) ? null : Alloy.Collections[model].get(index).toJSON();

    Alloy.createController(path, {
        method: (Alloy.Globals.candidate === null && Alloy.Globals.guest === true) ? 'POST' : 'GET',
        data,
        sectors
