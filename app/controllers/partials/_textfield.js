...
if (args.isDate) {
  $.container2.addEventListener("click", handleDate);
  if (OS_ANDROID) {
    $.container2.add(
      $.UI.create("View", {
        touchFeedback: true,
...
function handleList(list) {
  ...
  if (OS_ANDROID) {
    if (list) {
      ...
      $.background.backgroundColor = Alloy.CFG.COLORS.white;
  ...
  if (OS_IOS) {
    ...
    if (first) {
      ...
      $.textfield.value = first.value;
      $.textfield.val = first.val;
    }
  }
...
function getValue() {
  if (args.isDate) {
	 const date = ($.textfield.date) ? Alloy.Globals.moment($.textfield.date).format() : null;
    return date;
  } else {
    if (args.isList) {
...
function setKeyboardType(keyboard) {
