...
function saveSettings() {
    ...
    Alloy.Globals.Api.updateNotificationSettings({
        ...
        settings["news"] ? fcm.subscribeToTopic("news") : fcm.unsubscribeFromTopic("news");
        settings["events"] ? fcm.subscribeToTopic("events") : fcm.unsubscribeFromTopic("events");
...
function renderView() {
    ...
    if (!_.isNull(user) && !user.affiliate.visible) {
        content.push({
            name: {
                text: "Mostrar de nuevo mi empresa en listados"
            },
            icon: {
                text: "\uf06e"
            }
...
function setAffiliateVisible() {
    Alloy.Globals.Api.updateAffiliate({
        ...
        if (!response.success) {
            alert(`Hubo un problema al actualizar el afiliado. Contacte con ${Alloy.CFG.global.mail}`, "Error");
            return;
        }
...
function makeChangeresetRequest() {
    ...
    dialog.addEventListener("click", function(e) {
        ...
        Alloy.Globals.Api.resetPassword({
            ...
            Alloy
                .Globals
                .showMessage(response.message, "Cambio de contraseña");
        });
    });
    dialog.show();
}
