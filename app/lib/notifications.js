(function () {
	...
	var _exports = function () {
		if (OS_IOS) {
			Ti.App.iOS.addEventListener('usernotificationsettings', function eventUserNotificationSettings() {
				...
				Ti.Network.registerForPushNotifications({
					...
					error: function (res) {
						console.log('Error Ti.Network!: ' + JSON.stringify(res, null, 2));
					},
			...
			Ti.App.iOS.addEventListener('remotenotificationaction', function(event) {
				...
				console.log('remotenotificationaction. Handle background notification action click');
				console.log(JSON.stringify(event, null, 2));
			});
		} else {
		...
		function onToken(e) {
			console.log(`Alloy.Globals.deviceToken = ${Alloy.Globals.deviceToken}`, `e.fcmToken = ${e.fcmToken}`);
		...
		function onMessage(response) {
			Alloy.Globals.events.trigger('handle_notification', (OS_IOS) ? response.data : response.message.data);
		}
