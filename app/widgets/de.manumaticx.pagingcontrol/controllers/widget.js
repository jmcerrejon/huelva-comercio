...
_.each(['tabs', 'findScrollableView'], function (key) {
    ...
    try {
        ...
        delete args[key];
        Ti.API.error(
            "Unable to set argument '" + key + "'. It must be boolean."
        );
    }
});
...
if (args.__parentSymbol && args.findScrollableView) {
    args.__parentSymbol.children.length > 0 &&
        ($.scrollableView = _.find(
            args.__parentSymbol.children,
            function (child) {
...
function onScroll(e) {
    if (previousPage !== e.currentPage) {
        ...
        $.tabsCtrl.selectFont(e.currentPage);
    ...
    if (width < tabsWidth) {
        var offset = tabSpace - args.scrollOffset,
            ...
            newOffset = OS_IOS
                ...
                : offsetDp;
        if (OS_ANDROID) {
            // newOffset = measurement.dpToPX(offsetDp);
        }
...
function adjustePositions() {
    var totalWidth = args.tabs
        ? $.tabsCtrl.getWidth()
        : $.pagingcontrol.size.width;
